...
class IMMEstimator(object):
    ...
    def m_predict(self):   #Model prediction
    ...
    def x_predict(self, TMs):
        self.Transiton_Matrixs = TMs
        # perform predict step using the mixed initial conditions
        for i, (f, Fm) in enumerate(zip(self.filters, self.Transiton_Matrixs)):
            # propagate using the mixed state estimate and covariance
            f.x = self.xs[i]
            f.P = self.Ps[i]      
            f.predict(fx_args=Fm)
